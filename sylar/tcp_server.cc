...
sylar::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout=
        sylar::Config::Lookup("tcp_server.read_time",(uint64_t)(60*1000*2),
        "tcp_server read timeout");

static sylar::Logger::ptr g_logger=SYLAR_LOG_NAME("system");

TcpServer::TcpServer(sylar::IOManager* worker,sylar::IOManager* aworker)
    :m_worker(worker)
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs,
    ...
    for(auto& addr:addrs){
        ...
        if(!sock->listen()){
            SYLAR_LOG_ERROR(g_logger)<<"listren fail errno="
                <<errno<<" errstr="<<strerror(errno)
                <<" addr=["<<addr->toString()<<"]";
                fails.push_back(addr);
                continue;
