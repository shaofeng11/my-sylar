...
void ByteArray::writeFint16(int16_t value){
    //如果字节序和机器的字节序不同，需要把要写的值改成机器的字节序
...
void ByteArray::writeUint32(uint32_t value){
    ...
    uint8_t tmp[5];
    uint8_t i=0;
    //0x80 的二进制表示是 1000 0000  所以 0x80 | uint8(value&0x7F)是在取出的value的后7个bit位前加上1（msb）
    //说明后面还有数据
    //值大于等于1个字节
    //0x80 在编程中通常表示一个字节(Byte)的十六进制值,
    while(value>=0x80){
...
//先读，若和机器的字节序一样，则直接返回，若不一样，先转换字节序
#define XX(type) \
    type v; \
    read(&v,sizeof(v)); \
    if(m_endian==SYLAR_BYTE_ORDER){ \
...
void ByteArray::addCapacity(size_t size){
    ...
    if(old_cap>=size){
        //SYLAR_LOG_DEBUG(g_logger)<<"old_cap>=size";
        return;
    }
    //还需要多大空间
    size=size-old_cap;
...
std::string ByteArray::toString() const{
    ...
    //从mposition开始读str,size()个字节放到str[0]
