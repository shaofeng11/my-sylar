...
HttpResponse::ptr HttpConnection::recvResponse(){
    ...
    do{
        ...
        len+=offset;
        data[len]='\0';
        size_t nparser=parser->excute(data,len,false);
        if(parser->hasError()){
            close();
            return nullptr;
        }
    ...
    if(client_parser.chunked){
        ...
        do{
           do{
                ...
                len+=rt;
                data[len]='\0';
                size_t nparser=parser->excute(data,len,true);
                if(parser->hasError()){
                    close();
                    return nullptr;
                }
                len-=nparser;
                if(len==(int)buff_size){//一个数据都没解
                    close();
                    return nullptr;
                }
...
HttpResult::ptr HttpConnection::DoRequest(HttpMethod method
                                ...
                                ,const std::map<std::string,std::string>& headers
                                ,const std::string& body){
    //make_shared 和new一样的作用//它能够在动态内存中，分配并初始化一个对象，然后返回指向此对象的shared_ptr;
    ...
    bool has_host=false;
    for(auto& i:headers){
        //如果header里有connection信息，且是keep-alive，说明client需要常连接，将req的Close设置为false
        ...
        //若header中有host，且不为空，has_host==true
        if(!has_host && strcasecmp(i.first.c_str(),"host")==0){
            has_host=!i.second.empty();
        }
        req->setHeader(i.first,i.second);
    }
    //如果header中没有host信息，就用uri的host信息
    if(!has_host){
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req
    ...
    if(!sock){
        return std::make_shared<HttpResult>((int)HttpResult::Error::CREATE_SOCKET_ERROR,
                                            nullptr,"create fail: "+addr->toString()
...
HttpConnection::ptr HttpConnectionPool::getConnection(){
    ...
    while(!m_conns.empty()){
        ...
        //一个连接的创建时间+最大存活时间比当前时间还大，说明连接失效了
    ...
    //std::placeholder，当分批付给函数参数的过程中，如果还没有赋值的参数，
    //使用 std::placeholder 进行占位，意思时该位置的参数现在还不清楚，后期再赋值。
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req
    ...
    if(rt==0){
        return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_CLOSE_BY_PEER,
                                            nullptr,"send request closed by peer: "+sock->getRomoteAddress()->toString());
    }
    if(rt<0){
        return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_SOCKET_ERROR,
                                            nullptr,"send request socket error,errno= "+std::to_string(errno)
    ...
    if(!rsp){
        return std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT,
                                            nullptr,"recv response timeout: "+sock->getRomoteAddress()->toString() 
                                            +" timeout_ms"+std::to_string(timeout_ms));
    }
    //return std::shared_ptr<HttpResult>((int)HttpResult::Error::OK,rsp,"ok");
    return std::make_shared<HttpResult>((int)HttpResult::Error::OK,rsp,"ok");
}
