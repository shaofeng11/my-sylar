 ...
 * @return HTTP方法枚举
 */
HttpMethod StringToHttpMethod(const std::string& m);//描述转成数字
...
const char* HttpMethodToString(const HttpMethod& m);//数字转成描述
...
public:
    ...
    HttpRequest(uint8_t version=0x11,bool close=true);//默认非 常连接
    ...
    std::string getParams (const std::string& key,const std::string& def="");
    std::string getCookie (const std::string& key,const std::string& def="");


    //改变headers中的一个值
    void setHeader(const std::string& key,const std::string& def="") ;
    void setParam(const std::string& key,const std::string& def="") ;
    void setCookie(const std::string& key,const std::string& def="") ;
...
private:
    HttpMethod m_method;
    uint8_t m_version;//0x10: 1.0,  0x11: 1.1,  0x21: 2.1 
    bool m_close;
